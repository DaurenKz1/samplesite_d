...
class BbInline(admin.StackedInline):
    model = Bb
    # extra = 1
    vernose_name = 'Обьявление рубрики'
    verbose_name_plural = 'Обьявления рубрики'
    def get_extra(selfself, request, obj = None, **kwargs):
        if obj:
            return 1
        return 3
...
class BbAdmin(admin.ModelAdmin):
    ...
    #django grappelli  for searching '^title'-from the begnning '=title'-identical
    ...
    # def get_list_display(self, request):    #     ld = ['title', 'content', 'price']
    #     if request.user.is_superuser:    #         ld += ['published', 'rubric']
    #         # ld.append('published')    #     return ld
    ...
    def view_on_site(self, rec):
        return reverse('bboatd:detail', kwargs={'pk': rec.pk})
    ...
    def discount(self, request, queryset):
        ...
        for rec in queryset:
            rec.price = f / 2
            rec.save()
        self.message_user(request, message='Действие выполнено!')
