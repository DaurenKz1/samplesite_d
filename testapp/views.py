...
def test_email(request):
    ...
    # em = EmailMessage(subject='Аш новый пароль', body='Ваш новый пароль находится во вложении', attachments=[('password.text', '123456789', 'text/plain')],  to=['user@supersite.kz'])
    # em = EmailMessage(subject='Запошенный вами файл', body='Получите файл',  to=['user@supersite.kz'])
    # em.attach_file(os.path.join(BASE_DIR, 'tmp', 'file.txt'))
    # context = {'user' : 'Криштиану Роналду'}
    # s = render_to_string('email/letter.txt', context)
    #
    # em = EmailMessage(subject='Оповещение', body=s, to=['cr7@supersite.kz'])
    # # em.encoding('utf-8')
    # em.send()

    # con = get_connection()
    # con.open()
    # em_1 = EmailMessage(subject='Test1', body='Test1', to=['user1@supersite.kz'], connection=con)
    # em1.send()
    # em_2 = EmailMessage(subject='Test2', body='Test2', to=['user2@supersite.kz'], connection=con)
    # em2.send()
    # em_3 = EmailMessage(subject='Test3', body='Test3', to=['user3@supersite.kz'], connection=con)
    # em3.send()
    #
    # con.close()
    ...
    # con = get_connection()
    # con.open()
    # em_1 = EmailMessage(subject='Test1', body='Test1', to=['user1@supersite.kz'])
    #
    # em_2 = EmailMessage(subject='Test2', body='Test2', to=['user2@supersite.kz'])
    #
    # em_3 = EmailMessage(subject='Test3', body='Test3', to=['user3@supersite.kz'])
    #
    # con.send_messages([em_1, em_2, em_3])
    # con.close()

    # em = EmailMultiAlternatives(subject='Test', body='Test', to=['user@supersite.kz'])
    # em.attach_alternative('<h1>Test</h1>', 'text/html')
    # em.send()

    # send_mail('Test', 'Test!!!', 'webmaster@supersite.kz', ['user@othersite.kz'], html_message='<h1>Test</h1>')
    msg1 = ('Подписка', 'Подвердите покупку', webmaster)
